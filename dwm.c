...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
                if (ev->x < ble - blw) {
                        ...
                        click = ClkWinTitle;
                else if ((x = wsbar - lrpad / 2 - ev->x) > 0 && (x -= wstext - lrpad / 2) <= 0) {
...
drawbar(Monitor *m)
{
   ...
   int wbar = m->ww;
	...
	x = 0;
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]); drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsNorm]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		if (occ & 1 << i)
			drw_rect(drw, x + boxs, boxs, boxw + 1, boxw + 1,
...
setfullscreen(Client *c, int fullscreen)
{
	if (fullscreen && !c->isfullscreen) {
		...
		c->bw = 0;
		c->isfloating = 1;
		resizeclient(c, c->mon->mx, c->mon->my - 1, c->mon->mw, c->mon->mh + 1);
		XRaiseWindow(dpy, c->win);
